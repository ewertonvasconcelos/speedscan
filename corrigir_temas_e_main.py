...
# 1. Corrigir core/config.py
config_path = Path("core/config.py")
backup_config = Path("core/config.py.bak_temas")
shutil.copy2(config_path, backup_config)
print(f"Backup de config.py criado: {backup_config}")

with open(config_path, 'r', encoding='utf-8') as f:
    config_content = f.read()

# Substituir a seção THEMES pela versão completa com os temas desejados
import re
pattern = r'THEMES = \{.*?\}'
new_themes = '''THEMES = {
...
config_content = re.sub(pattern, new_themes, config_content, flags=re.DOTALL)

# Garantir que DEFAULT_CONFIG["theme"] seja "techneon"
config_content = re.sub(r'"theme": ".*?"', '"theme": "techneon"', config_content)

with open(config_path, 'w', encoding='utf-8') as f:
    f.write(config_content)
print("config.py atualizado com os temas corretos.")

# 2. Substituir core/main.py pela versão final e estável (a que forneci anteriormente)
main_path = Path("core/main.py")
backup_main = Path("core/main.py.bak_final")
shutil.copy2(main_path, backup_main)
print(f"Backup de main.py criado: {backup_main}")

# Conteúdo do main.py corrigido (já fornecido no passo anterior, mas vou reutilizar)
...
class SpeedScan(ctk.CTk):
    def __init__(self):
        ...
        self.detail_buttons = {}
        self.logs = {}
        self.result_labels = {}  # Para armazenar labels de resultados nos cards
    ...
    def update_theme_vars(self):
        # Usa o tema padrão do config (techneon) se o salvo não existir
    ...
    def _fill_config(self, parent):
        ...
        # Nomes dos temas conforme solicitado
    ...
    def run_card_action(self, cmd, tag, is_dns):
        ...
        if not log:
            return
        log.delete("1.0", "end")
        # Esconder botão detalhes se estiver visível
    ...
    def _show_details_button(self, tag):
        ...
        if btn:
            btn.configure(text=self._("Detalhes ▼"))
...
print("main.py substituído pela versão estável.")

print("Tudo pronto. Execute 'python -m core.main' para testar.")
