#!/bin/bash
# Script para recriar todos os 24 módulos do SpeedScan no host

set -e

cd ~/speedscan/speedscan

# Backup da core antiga
if [ -d core ]; then
    mv core core.bak.$(date +%s)
    echo "✅ Backup da core antiga criado."
fi
...
© 2026 Ewerton Vasconcelos. {self._("All rights reserved.")}"""
    ...
    def _run_ping(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Pinging google.com...'; ping -c 4 google.com"], log, tag="ping")
    ...
    def _run_ethtool(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Running ethtool...'; ethtool eth0"], log, tag="ethtool")

    def _run_dhclient(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Renewing IP via dhclient...'; sudo dhclient -v"], log, use_sudo=True, tag="dhclient")
    ...
    def _run_traceroute(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Running traceroute...'; traceroute google.com"], log, tag="traceroute")

    def _run_wifi(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Wi-Fi info...'; iwconfig"], log, tag="wifi")

    def _run_testdns(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Testing DNS...'; nslookup google.com"], log, tag="testdns")
    ...
    def _run_pci(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Listing PCI devices...'; lspci"], log, tag="pci")

    def _run_update(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Updating package list...'; sudo apt update"], log, use_sudo=True, tag="update")

    def _run_usb(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Listing USB devices...'; lsusb"], log, tag="usb")

    def _run_modules(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Listing kernel modules...'; lsmod"], log, tag="modules")
    ...
    def _run_firmware(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Checking firmware messages...'; dmesg | grep -i firmware"], log, shell=True, tag="firmware")

    def _run_video_drv(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Detecting video drivers...'; lspci | grep -i vga"], log, shell=True, tag="video_drv")

    def _run_net_drv(self, log):
        self._run_subprocess(["sh", "-c", "echo 'Detecting network drivers...'; lspci | grep -i network"], log, shell=True, tag="net_drv")
...
class AIProactive:
    ...
    def get_summary(self):
        ...
        return "\n".join(lines)
EOF
echo "✅ core/ai_proactive.py criado."
...
EOF
echo "✅ core/browser_cleaner.py criado."
...
EOF
echo "✅ core/health_score.py criado."
...
EOF
echo "✅ core/historical_metrics.py criado."
...
EOF
echo "✅ core/lan_scanner.py criado."
...
EOF
echo "✅ core/process_manager.py criado."
...
EOF
echo "✅ core/security_scanner.py criado."
...
class SmartMonitor:
    ...
    def get_status_color(self):
        try:
            ...
            return "yellow"
EOF
echo "✅ core/smart_monitor.py criado."
...
class SpeedTester:
    ...
    def format_result(self, result=None) -> str:
        ...
        return "\n".join(lines)
EOF
echo "✅ core/speed_test.py criado."
...
class TemperatureMonitor:
    ...
    def get_all_temperatures(self):
        ...
        return temps
EOF
echo "✅ core/temperature_monitor.py criado."
...
class TrashManager:
    ...
    def get_trash_size(self):
        ...
        return total
EOF
echo "✅ core/trash_manager.py criado."
...
class WindowsCleaner:
    ...
    def _run_command(self, cmd: str, log_callback: Optional[Callable[[str], None]] = None) -> bool:
        try:
            ...
            return False
EOF
echo "✅ core/windows_cleaner.py criado."
...
class CookieManager:
    ...
    def delete_cookies_except(self, browser_key: str, keep_domains: List[str]) -> bool:
        ...
        try:
            ...
            return False
EOF
echo "✅ core/cookie_manager.py criado."

# ==================== __init__.py ====================
touch core/__init__.py
echo "✅ core/__init__.py criado."

# ==================== traduções mínimas ====================
echo "🌐 Recriando traduções mínimas..."
rm -rf locale
...
echo ""
echo "🎉 TODOS OS 24 MÓDULOS FORAM RECRIADOS!"
echo "Agora entre no container e execute o programa:"
echo "   docker start -ai speedscan-dev"
echo "   cd /app"
