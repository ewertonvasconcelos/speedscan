...
class SlotWidget(ctk.CTkFrame):
    ...
    def __init__(self, parent, slot_index, widget_type, app_instance, **kwargs):
        print(f"DEBUG SlotWidget.__init__: type(app_instance) = {type(app_instance)}")
        ...
        # Debug log (agora com indentação correta)
    ...
    def update_content(self):
        ...
        print(f"DEBUG SlotWidget.update_content: self.app type = {type(self.app)}, callback_name={callback_name}")
        try:
            callback = getattr(self.app, callback_name)
        except AttributeError as e:
            print("DEBUG: AttributeError:", e, "self.app type =", type(self.app), "callback_name =", callback_name)
            print("DEBUG: self.app dir =", dir(self.app)[:10], "...")
            # Tentar obter do tipo
            callback = getattr(type(self.app), callback_name, None)
            if callback:
                # Se for método, precisamos do self
                callback = callback.__get__(self.app, type(self.app))
            else:
                raise
...
class Dashboard(ctk.CTkFrame):
    ...
    def __init__(self, parent, app_instance, **kwargs):
        print(f"DEBUG Dashboard.__init__: type(app_instance) = {type(app_instance)}")
