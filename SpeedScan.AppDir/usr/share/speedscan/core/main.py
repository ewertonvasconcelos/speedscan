    ...
    def widget_disks(self, frame, tag):
        ...
        if is_small:
            ...
            label = ctk.CTkLabel(
                ...
                text=f"{root.percent}%", 
                font=("Inter", 20, "bold"),
                text_color=get_usage_color(root.percent)
            )
            label.pack(anchor="center", padx=5, pady=5)
        else:
            # Big widget - disk details
            # Root disk
            root_percent = root.percent
            root_color = get_usage_color(root_percent)
            ...
            root_label.pack(anchor="w", padx=10, pady=(10, 2))
            
            root_progress = ctk.CTkProgressBar(frame, orientation="horizontal")
            root_progress.set(root_percent / 100)
            root_progress.configure(
                progress_color=root_color,
                fg_color="#3B3B3B",
                height=15
            )
            root_progress.pack(fill="x", padx=10, pady=2)
            
            root_details = ctk.CTkLabel(
                frame,
                text=f"{root.used // (1024**3)} GB / {root.total // (1024**3)} GB ({root_percent}%)",
                font=("Inter", 10),
                text_color=("gray40", "gray70")
            )
            root_details.pack(anchor="w", padx=10, pady=2)
            
            # Home disk
            home_percent = home.percent
            home_color = get_usage_color(home_percent)
            ...
            home_label.pack(anchor="w", padx=10, pady=(5, 2))
            
            home_progress = ctk.CTkProgressBar(frame, orientation="horizontal")
            home_progress.set(home_percent / 100)
            home_progress.configure(
                progress_color=home_color,
                fg_color="#3B3B3B",
                height=15
            )
            home_progress.pack(fill="x", padx=10, pady=2)
            
            home_details = ctk.CTkLabel(
                frame,
                text=f"{home.used // (1024**3)} GB / {home.total // (1024**3)} GB ({home_percent}%)",
                font=("Inter", 10),
                text_color=("gray40", "gray70")
            )
            home_details.pack(anchor="w", padx=10, pady=(2, 10))
