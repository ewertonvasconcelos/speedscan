...
backup = Path("core/main.py.bak_final2")
shutil.copy2(arquivo, backup)
print(f"Backup criado: {backup}")

conteudo = arquivo.read_text()

# Padrão para encontrar a função round_image atual (do início ao fim)
padrao = re.compile(
    r'(def round_image\(self, path, size=\(96,96\), radius=20\):.*?)(?=\n\s*def|\Z)',
    re.DOTALL
)

# Versão correta com indentação de 4 espaços (padrão Python)
...
if padrao.search(conteudo):
    ...
    print("Função round_image substituída com sucesso (indentação 4 espaços).")
else:
