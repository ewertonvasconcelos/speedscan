...
# Executar no diretório raiz do projeto (ex: ~/speedscan/speedscan)

set -e

echo "🚀 Iniciando recriação completa do SpeedScan (todos os módulos)..."

# Remove core anterior se existir (faz backup)
if [ -d "core" ]; then
    mv core "core.bak.$(date +%s)"
    echo "✅ Backup da core antiga salvo."
fi

mkdir -p core
echo "✅ Pasta core criada."
