...
class SpeedScan(ctk.CTk):
    ...
    def _fill_settings(self, parent):
        ...
        ctk.CTkOptionMenu(f_lang, values=lang_values, variable=self.lang_var, width=200,
                          fg_color=self.bg_color, button_color=self.acc_color, button_hover_color=self.light_bg).pack(anchor="center", pady=2)
...
© 2026 Ewerton Vasconcelos. {self._("All rights reserved.")}"""
    ...
    def widget_temps(self):
                ...
                for name, entries in temps.items():
                    if 'cpu' in name.lower() or 'core' in name.lower():
                        if entries:
                            ...
                            # Return dict with icon and value
                            return {
                                'icon': icon,
                                'value': f"{temp}°C",
                                'temp': temp
        ...
        # Fallback para quando não há dados de temperatura
        return {
            'icon': '🌡️',
            'value': 'N/A',
            'temp': None
    ...
    def widget_uptime(self):
        ...
        return {
            'icon': '⏳',
            'value': time_text
        }  # Dashboard adicionará ícone animado
    ...
    def widget_health(self):
        ...
        # Determine icon based on health score
        if health_score >= 80:
            icon = "❤️"  # Bom
    ...
    def widget_ram(self):
        ...
        return {
            'icon': '🧠',
            'value': f"{used_gb}GB / {total_gb}GB ({mem.percent}%)",
            'used': used_gb,
            'total': total_gb,
            'percent': mem.percent
        }
    ...
    def widget_battery(self):
        ...
        if battery is None:
            return {'percent': 0, 'plugged': False, 'status': 'No battery', 'icon': '🔋', 'color': '#888888'}
        
        percent = int(battery.percent)
        plugged = battery.power_plugged
        status = "Carregando" if plugged else "Descarregando"
        
        # Ícone baseado no percentual e status
        if plugged:
            icon = "🔌"  # Carregando
        elif percent >= 80:
            icon = "🔋"  # Cheio
