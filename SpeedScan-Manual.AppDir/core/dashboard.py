...
class SlotWidget(ctk.CTkFrame):
    ...
    def _build_cpu_ui(self, data):
        """Build CPU UI from data."""
        label = ctk.CTkLabel(self.content_frame, text=data, 
                            font=("Inter", 16, "bold"), text_color=self.app.acc_color,
                            wraplength=220, justify="center")
        label.pack(anchor="center", expand=True, padx=5, pady=5)
    
    def _build_ram_ui(self, data):
        """Build RAM UI from data."""
        label = ctk.CTkLabel(self.content_frame, text=data, 
                            font=("Inter", 16, "bold"), text_color=self.app.acc_color,
                            wraplength=220, justify="center")
        label.pack(anchor="center", expand=True, padx=5, pady=5)
    ...
    def _build_hostname_ui(self, data):
        """Build hostname UI from data."""
        # Data já está formatada como "🏠 hostname"
        label = ctk.CTkLabel(self.content_frame, text=data, 
                            font=("Inter", 16, "bold"), text_color=self.app.acc_color,
                            wraplength=220, justify="center")
        label.pack(anchor="center", expand=True, padx=5, pady=5)
    ...
    def _build_distro_ui(self, data):
        """Build distro UI from data."""
        # Data já está formatada como "🐧 Ubuntu"
        label = ctk.CTkLabel(self.content_frame, text=data, 
                            font=("Inter", 16, "bold"), text_color=self.app.acc_color,
                            wraplength=220, justify="center")
        label.pack(anchor="center", expand=True, padx=5, pady=5)
    ...
    def _build_kernel_ui(self, data):
        """Build kernel UI from data."""
        # Data já está formatada como "⚙️ 6.19"
        label = ctk.CTkLabel(self.content_frame, text=data, 
                            font=("Inter", 16, "bold"), text_color=self.app.acc_color,
                            wraplength=220, justify="center")
        label.pack(anchor="center", expand=True, padx=5, pady=5)
    ...
    def _build_temps_ui(self, data):
        """Build temperature UI from data."""
        # Data já está formatada como "🔥 91.0°C" ou "N/A"
        if isinstance(data, str):
            if "N/A" in data:
                label = ctk.CTkLabel(self.content_frame, text="🌡️ N/A", 
                                    font=("Inter", 16, "bold"), text_color=self.app.acc_color,
                                    wraplength=220, justify="center")
            else:
                label = ctk.CTkLabel(self.content_frame, text=data, 
    ...
    def _build_battery_ui(self, data):
        ...
        # Data já está formatada como "🔋 97%"
        if isinstance(data, str):
            if "N/A" in data:
                label = ctk.CTkLabel(self.content_frame, text="🔋 N/A", 
                                    font=("Inter", 16, "bold"), text_color=self.app.text_color,
                ...
                # Extrair percentual para cor
                try:
                    percent = int(data.split()[1].replace('%', ''))
                    color = self._get_battery_color(percent)
                except:
                    color = self.app.acc_color
                
                label = ctk.CTkLabel(self.content_frame, text=data, 
                                    font=("Inter", 16, "bold"), text_color=color,
                                    wraplength=220, justify="center")
        else:
            label = ctk.CTkLabel(self.content_frame, text=f"🔋 {data}", 
                                font=("Inter", 16, "bold"), text_color=self.app.text_color,
...
class SmallWidget(ctk.CTkFrame):
    ...
    def __init__(self, parent, widget_type, app_instance, on_click=None, **kwargs):
        ...
        self.title_label.grid(row=0, column=0, pady=(5, 0), sticky="ew")
    ...
    def _build_small_widget_ui(self, data):
        ...
        if widget_id == "disks":
            ...
            self._build_small_health_ui(data)
        elif widget_id == "uptime":
            self._build_small_uptime_ui(data)
        else:
            # Default: show data as text
            label = ctk.CTkLabel(self.content_frame, text=str(data), 
    ...
    def _build_small_uptime_ui(self, data):
        """Build small uptime UI with animated icon."""
        ...
        # Iniciar animação do uptime
        self._uptime_anim_counter = 0
        self._uptime_data = uptime_data  # Guardar dados para animação
        self._animate_uptime()
        
        label = ctk.CTkLabel(self.content_frame, text=f"{icon} {uptime_data}", 
                            font=("Arial", 10, "bold"), text_color=self.app.acc_color,
    ...
    def _animate_uptime(self):
        """Animate uptime icon between ⏳ and ⌛"""
        if hasattr(self, '_uptime_label') and self._uptime_label.winfo_exists():
            self._uptime_anim_counter = (self._uptime_anim_counter + 1) % 2
            new_icon = "⌛" if self._uptime_anim_counter == 0 else "⏳"
            new_text = f"{new_icon} {self._uptime_data}"
            self._uptime_label.configure(text=new_text)
            # Schedule next update after 1 second
            self.after(1000, self._animate_uptime)
    
    def _build_small_health_ui(self, data):
        """Build small health UI from data."""
        if isinstance(data, dict):
    ...
    def _build_small_disks_ui(self, data):
        ...
        if isinstance(data, dict):
            ...
            value = data.get('value', 'N/A')
            display_text = f"{icon} {value}"
            color = self._get_usage_color(max(data.get('root', {}).get('percent', 0), data.get('home', {}).get('percent', 0)))
        else:
            # Fallback para dados inválidos
            display_text = "💾 N/A"
            color = self.app.text_color
    ...
    def _build_small_gpu_ui(self, data):
        ...
        if isinstance(data, dict):
            icon = data.get('icon', '🎮')
            value = data.get('short_value', data.get('value', 'N/A'))
            display_text = f"{icon} {value}"
        else:
            # Formato antigo string
            ...
            if "Intel" in gpu_name:
                ...
                gpu_name = "AMD"
            elif len(gpu_name) > 12:
                gpu_name = gpu_name[:12] + "..."
            display_text = f"🎮 {gpu_name}"
    ...
    def _build_small_battery_ui(self, data):
        ...
        if isinstance(data, dict):
            ...
            if "N/A" in data:
                label = ctk.CTkLabel(self.content_frame, text="🔋 N/A", 
                                    font=("Arial", 10, "bold"), text_color=self.app.text_color,
                                    wraplength=160, justify="center")
            else:
                # Extrair percentual para cor
                try:
    ...
    def _build_small_temps_ui(self, data):
        ...
        if isinstance(data, dict):
            ...
            if "N/A" in data:
                ...
                # Extrair temperatura para cor
                try:
                    temp_str = data.split()[1]  # Pega "91.0°C"
