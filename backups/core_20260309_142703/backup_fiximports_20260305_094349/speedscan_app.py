...
def get_config():
    if os.path.exists(config.config.CONFIG_FILE):
        try:
            with open(config.config.CONFIG_FILE, "r") as f:
                return json.load(f)
        except:
            pass
    return {
...
class SpeedScan(ctk.CTk):
    ...
    def create_sidebar(self):
        ...
        icon_image = None
        if os.path.exists(config.config.ICON_PATH):
            icon_image = self.round_image(config.config.ICON_PATH, size=(96, 96), radius=20)
    ...
    def create_agente_frame(self, parent):
        ...
        for idx, ia in enumerate(config.config.AI_SUGGESTIONS):
            row, col = divmod(idx, 3)
            card = ctk.CTkFrame(grid, fg_color=self.bg_color, corner_radius=10, border_width=1, border_color=self.acc_color)
            card.grid(row=row, column=col, padx=10, pady=10, sticky="nsew")
            card.grid_propagate(False)
    ...
    def create_config_frame(self, parent):
        ...
        ctk.CTkLabel(log_frame, text=f"Logs salvos em: {config.config.LOG_DIR}", font=("Inter", 11), text_color=self.text_color).pack(side="left", padx=5)
    ...
    def open_logs_folder(self):
        try:
            if self.SO == "Windows":
                os.startfile(config.config.LOG_DIR)
            elif self.SO == "Darwin":
                subprocess.run(["open", config.config.LOG_DIR])
            else:
                subprocess.run(["xdg-open", config.config.LOG_DIR])
        except Exception as e:
    ...
    def create_system_schedule(self, schedule):
        ...
        agent_cmd = f"python3 {config.config.AGENT_SCRIPT}"
    ...
    def _schedule_linux(self, schedule, cmd):
        ...
        full_cmd = f"{cmd} >> {config.config.LOG_DIR}/schedule.log 2>&1"
