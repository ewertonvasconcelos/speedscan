...
class SpeedScan(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"SpeedScan {config.config.VERSION}")
    ...
    def _check_first_run(self):
        if self.config == config.config.DEFAULT_CONFIG:
    ...
    def _restore_window_state(self):
        ws = self.config.get("window_state", config.config.DEFAULT_CONFIG["window_state"])
    ...
    def _save_window_state(self):
        ...
        try:
            ...
            ws = config.config.DEFAULT_CONFIG["window_state"]
    ...
    def _load_config(self):
        if config.config.CONFIG_FILE.exists():
            try:
                with open(config.config.CONFIG_FILE) as f:
                    cfg = json.load(f)
                    if 'window_state' not in cfg:
                        cfg['window_state'] = config.config.DEFAULT_CONFIG['window_state']
                    if 'simple_mode' not in cfg:
                        cfg['simple_mode'] = config.config.DEFAULT_CONFIG['simple_mode']
                    if 'expert_level' not in cfg:
                        cfg['expert_level'] = config.config.DEFAULT_CONFIG['expert_level']
                    if 'ai' not in cfg:
                        cfg['ai'] = config.config.DEFAULT_CONFIG['ai']
                    return cfg
            except:
                pass
    ...
    def _save_config(self):
        with open(config.config.CONFIG_FILE, "w") as f:
            json.dump(self.config, f, indent=2)

    def update_theme_vars(self):
        t = config.config.THEMES.get(self.config["theme"], config.config.THEMES["default"])
    ...
    def _build_sidebar(self):
        ...
        top.pack(pady=(30, 5))
        icon = self.round_image(str(config.config.ICON_PATH)) if config.config.ICON_PATH.exists() else None
        if icon:
    ...
    def _fill_config(self, parent):
        ...
        self.lang_var = ctk.StringVar(value=config.config.LANGUAGES.get(self.config.get("language","pt_BR"), "Português Brasileiro"))
        ctk.CTkOptionMenu(f_lang, values=list(config.config.LANGUAGES.values()), variable=self.lang_var, width=300, cursor="left_ptr").pack(anchor="w")
        ...
        self.scale_var = ctk.StringVar(value=config.config.SCALES.get(self.config.get("ui_scale","auto"), "Automático"))
        ctk.CTkOptionMenu(f_scale, values=list(config.config.SCALES.values()), variable=self.scale_var, width=300, cursor="left_ptr").pack(anchor="w")
    ...
    def save_schedule_config(self):
        ...
        self._save_config()
        scheduler = Scheduler(self.SO, config.LOG_DIR, config.config.AGENT_SCRIPT)
    ...
    def apply_config(self):
        ...
        for k,v in config.config.LANGUAGES.items():
            if v == self.lang_var.get():
                self.config["language"] = k; break
        for k,v in config.config.SCALES.items():
    ...
    def _fill_sobre(self, parent):
        ...
        ctk.CTkLabel(card, text=f"Versão {config.config.VERSION}", font=("Inter",14), text_color="#888888").pack()
