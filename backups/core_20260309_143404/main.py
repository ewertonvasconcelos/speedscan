...
class SpeedScan(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        top.pack(pady=(15, 0))  # Espaço da logo até o topo (15px), 0 abaixo
        ...
        center.pack(expand=False, fill="x", pady=(38, 15))  # Espaço entre a logo e o primeiro botão (38px) e entre o último botão e a borda inferior (38px)
        ...
        # Botões inferiores (configurações e sobre) - agora integrados ao center
        for icon, text, target in [("⚙", "Configurações", "config"), ("ℹ", "Sobre", "sobre")]:
            btn = self._sidebar_btn(center, icon, text, target)
            self.sidebar_buttons[target] = btn

        # Spacer (height 0, apenas para organização)
    ...
    def _fill_config(self, parent):
        ...
        ctk.CTkLabel(f_lang, text="Idioma da Interface", font=("Inter",14), text_color=self.text_color).pack(anchor="w")
        ...
        ctk.CTkLabel(f_tab, text="Abrir arquivo em", font=("Inter",14), text_color=self.text_color).pack(anchor="w")
        ...
        ini_radio = ctk.CTkRadioButton(f_level, text="Iniciante (apenas funções básicas)", variable=self.level_var, value=1, cursor="hand2")
        ini_radio.pack(anchor="w", pady=2)
        inter_radio = ctk.CTkRadioButton(f_level, text="Intermediário (básicas + algumas avançadas)", variable=self.level_var, value=2, cursor="hand2")
        inter_radio.pack(anchor="w", pady=2)
        avan_radio = ctk.CTkRadioButton(f_level, text="Avançado (todas as funções)", variable=self.level_var, value=3, cursor="hand2")
        avan_radio.pack(anchor="w", pady=2)
        ...
        ctk.CTkLabel(freq_frame, text="Frequência:", font=("Inter",12), text_color=self.text_color).pack(side="left", padx=5)
        ...
        freq_menu = ctk.CTkOptionMenu(freq_frame, values=["daily", "weekly", "monthly", "custom"],
                                        variable=self.schedule_freq_var, command=self.update_schedule_visibility, width=150, cursor="left_ptr")
        freq_menu.pack(side="left", padx=5)

        time_frame = ctk.CTkFrame(self.schedule_options_frame, fg_color="transparent")
        time_frame.pack(fill="x", pady=5)
        ctk.CTkLabel(time_frame, text="Horário (HH:MM):", font=("Inter",12), text_color=self.text_color).pack(side="left", padx=5)
        ...
        ctk.CTkLabel(self.weekday_frame, text="Dia da semana:", font=("Inter",12), text_color=self.text_color).pack(side="left", padx=5)
        self.schedule_weekday_var = ctk.StringVar(value=self.config.get("schedule", {}).get("day_of_week", "monday"))
        weekday_menu = ctk.CTkOptionMenu(self.weekday_frame, values=["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"],
        ...
        ctk.CTkLabel(self.monthday_frame, text="Dia do mês:", font=("Inter",12), text_color=self.text_color).pack(side="left", padx=5)
        self.schedule_monthday_var = ctk.IntVar(value=self.config.get("schedule", {}).get("day_of_month", 1))
        monthday_entry = ctk.CTkEntry(self.monthday_frame, textvariable=self.schedule_monthday_var, width=50)
        ...
        ctk.CTkLabel(self.custom_interval_frame, text="Intervalo (dias):", font=("Inter",12), text_color=self.text_color).pack(side="left", padx=5)
        self.schedule_interval_var = ctk.IntVar(value=self.config.get("schedule", {}).get("interval_days", 7))
        interval_entry = ctk.CTkEntry(self.custom_interval_frame, textvariable=self.schedule_interval_var, width=50)
        interval_entry.pack(side="left", padx=5)

        tasks_frame = ctk.CTkFrame(self.schedule_options_frame, fg_color="transparent")
        tasks_frame.pack(fill="x", pady=5)
        ctk.CTkLabel(tasks_frame, text="Tarefas a executar:", font=("Inter",12,"bold"), text_color=self.acc_color).pack(anchor="w", pady=5)
        ...
        for key, label in task_list:
            var = ctk.BooleanVar(value=key in saved_tasks)
            cb = ctk.CTkCheckBox(tasks_frame, text=label, variable=var, onvalue=True, offvalue=False, cursor="hand2")
            cb.pack(anchor="w", padx=20, pady=2)
            self.schedule_tasks[key] = var

        elevated_frame = ctk.CTkFrame(self.schedule_options_frame, fg_color="transparent")
        elevated_frame.pack(fill="x", pady=5)
        self.schedule_elevated_var = ctk.BooleanVar(value=self.config.get("schedule", {}).get("elevated", False))
        elevated_check = ctk.CTkCheckBox(elevated_frame, text="Executar com privilégios de administrador (quando necessário)",
                                          variable=self.schedule_elevated_var, onvalue=True, offvalue=False, cursor="hand2")
        elevated_check.pack(anchor="w", pady=2)
        ...
        open_logs_btn = ctk.CTkButton(log_frame, text="Abrir pasta", command=self.open_logs_folder, width=100, height=25, cursor="hand2")
        open_logs_btn.pack(side="left", padx=5)

        save_schedule_btn = ctk.CTkButton(self.schedule_options_frame, text="Salvar configurações de agendamento",
                                            fg_color=self.acc_color, command=self.save_schedule_config, width=300, height=40, cursor="hand2")
        save_schedule_btn.pack(pady=15)
        ...
        apply_btn = ctk.CTkButton(parent, text="Aplicar", fg_color=self.acc_color, command=self.apply_config,
                                   width=200, height=40, cursor="hand2")
        apply_btn.pack(pady=20)

    # Aba Sobre (corrigida com aspas triplas)
...
© 2026 Ewerton Vasconcelos. Todos os direitos reservados."""
    ...
    def _fill_windows_cleaner(self, parent):
        if self.SO != "Windows" or self.windows_cleaner is None:
            ...
            return
        # ... (código completo da aba de limpeza do Windows pode ser adicionado depois)
    ...
    # =========================================================================
    # Métodos de agendamento (necessários para Configurações)
    # =========================================================================
