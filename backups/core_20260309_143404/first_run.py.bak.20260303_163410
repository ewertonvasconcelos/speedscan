...
class FirstRunWizard(ctk.CTkToplevel):
    def __init__(self, parent, config):
        ...
        iniciante_radio = ctk.CTkRadioButton(level_frame, text="Iniciante (apenas funções básicas)", variable=self.level_var, value="iniciante")
        iniciante_radio.pack(anchor="w", pady=2)
        intermediario_radio = ctk.CTkRadioButton(level_frame, text="Intermediário (funções básicas + algumas avançadas)", variable=self.level_var, value="intermediario")
        intermediario_radio.pack(anchor="w", pady=2)
        avancado_radio = ctk.CTkRadioButton(level_frame, text="Avançado (todas as funções, sem restrições)", variable=self.level_var, value="avancado")
