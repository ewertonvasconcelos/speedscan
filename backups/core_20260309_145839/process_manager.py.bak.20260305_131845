from core import config
#!/usr/bin/env python3
import logging
# Módulo de gerenciamento de processos (thread-safe com fila)
# Versão 1.0.0

import psutil
import time
import threading
import queue
from typing import List, Dict, Any, Optional

class ProcessManager:
    def __init__(self):
        self.sort_by = 'cpu_percent'
        self.reverse = True
        self.filter_term = ""
        self.update_interval = 2
        self._stop_event = threading.Event()
        self._thread = None
        self.callback_queue = queue.Queue()

    def get_process_list(self) -> List[Dict[str, Any]]:
        process_list = []
        for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent',
                                         'status', 'create_time', 'username', 'nice']):
            try:
                pinfo = proc.info
                pinfo['cpu_percent'] = round(pinfo['cpu_percent'] or 0, 1)
                pinfo['memory_percent'] = round(pinfo['memory_percent'] or 0, 1)
                create_time = pinfo['create_time']
                if create_time:
                    pinfo['create_time_str'] = time.strftime('%H:%M:%S', time.localtime(create_time))
                else:
                    pinfo['create_time_str'] = ''
                pinfo['nice'] = pinfo['nice'] if pinfo['nice'] is not None else 0
                process_list.append(pinfo)
except Exception as e:
    logging.error(f"Exceção em process_manager.py:38: {e}")
                continue

        if self.filter_term:
            term = self.filter_term.lower()
            process_list = [p for p in process_list if term in p['name'].lower()]

        process_list.sort(key=lambda x: x.get(self.sort_by, 0), reverse=self.reverse)
        return process_list

    def kill_process(self, pid: int) -> bool:
        try:
            proc = psutil.Process(pid)
            proc.terminate()
            gone, alive = psutil.wait_procs([proc], timeout=3)
            if alive:
                proc.kill()
            return True
except Exception as e:
    logging.error(f"Exceção em process_manager.py:56: {e}")
            logging.error(f"Erro ao matar processo {pid}: {e}")
            return False

    def set_nice(self, pid: int, nice_value: int) -> bool:
        try:
            proc = psutil.Process(pid)
            proc.nice(nice_value)
            return True
except Exception as e:
    logging.error(f"Exceção em process_manager.py:65: {e}")
            logging.error(f"Erro ao alterar nice do processo {pid}: {e}")
            return False

    def suspend_process(self, pid: int) -> bool:
        try:
            proc = psutil.Process(pid)
            proc.suspend()
            return True
except Exception as e:
    logging.error(f"Exceção em process_manager.py:74: {e}")
            logging.error(f"Erro ao suspender processo {pid}: {e}")
            return False

    def resume_process(self, pid: int) -> bool:
        try:
            proc = psutil.Process(pid)
            proc.resume()
            return True
except Exception as e:
    logging.error(f"Exceção em process_manager.py:83: {e}")
            logging.error(f"Erro ao resumir processo {pid}: {e}")
            return False

    def start_monitoring(self):
        self._stop_event.clear()
        if self._thread is None or not self._thread.is_alive():
            self._thread = threading.Thread(target=self._monitor_loop, daemon=True)
            self._thread.start()

    def stop_monitoring(self):
        self._stop_event.set()
        if self._thread:
            self._thread.join(timeout=1)

    def _monitor_loop(self):
        while not self._stop_event.is_set():
            procs = self.get_process_list()
            self.callback_queue.put(procs)
            time.sleep(self.update_interval)

    def set_sort(self, key: str, reverse: bool = True):
        self.sort_by = key
        self.reverse = reverse

    def set_filter(self, term: str):
        self.filter_term = term

    def set_update_interval(self, seconds: int):
        self.update_interval = max(1, seconds)
