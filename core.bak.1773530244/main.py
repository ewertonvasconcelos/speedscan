...
© 2026 Ewerton Vasconcelos. {self._("All rights reserved.")}"""
    ...
    def _run_ping(self, log):
        ...
        self._run_subprocess(["ping", "-c", "4", "google.com"], log, tag=tag, tag=tag)
    ...
    def _run_ethtool(self, log):
        ...
        self._run_subprocess(["ethtool", "eth0"], log, tag=tag, tag=tag)

    def _run_dhclient(self, log):
        log.insert("end", self._("Renewing IP via dhclient...\n"))
        self._run_subprocess(["sudo", "dhclient", "-v"], log, tag=tag, tag=tag, use_sudo=True)
    ...
    def _run_traceroute(self, log):
        ...
        self._run_subprocess(["traceroute", "google.com"], log, tag=tag, tag=tag)

    def _run_wifi(self, log):
        log.insert("end", self._("Wi-Fi info...\n"))
        self._run_subprocess(["iwconfig"], log, tag=tag, tag=tag)

    def _run_testdns(self, log):
        log.insert("end", self._("Testing DNS (google.com)...\n"))
        self._run_subprocess(["nslookup", "google.com"], log, tag=tag, tag=tag)
    ...
    def _run_pci(self, log):
        self._run_subprocess(["lspci"], log, tag=tag, tag=tag)

    def _run_update(self, log):
        log.insert("end", self._("Updating package list...\n"))
        self._run_subprocess(["sudo", "apt", "update"], log, tag=tag, tag=tag, use_sudo=True)

    def _run_usb(self, log):
        self._run_subprocess(["lsusb"], log, tag=tag, tag=tag)

    def _run_modules(self, log):
        self._run_subprocess(["lsmod"], log, tag=tag, tag=tag)
    ...
    def _run_firmware(self, log):
        self._run_subprocess(["dmesg", "|", "grep", "-i", "firmware"], log, tag=tag, tag=tag, shell=True)

    def _run_video_drv(self, log):
        self._run_subprocess(["lspci", "|", "grep", "-i", "vga"], log, tag=tag, tag=tag, shell=True)

    def _run_net_drv(self, log):
        self._run_subprocess(["lspci", "|", "grep", "-i", "network"], log, tag=tag, tag=tag, shell=True)
    ...
    def _change_dns(self, dns_ip, log):
        ...
        if hasattr(self, "action_mapper"):
            ...
            if cmd:
                self._run_subprocess(cmd, log, tag=tag, tag=tag, shell=True)
            else:
