...
#   ╚══════╝╚═╝     ╚══════╝╚══════╝╚═════╝ ╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═══╝
# =============================================================================
# Dashboard com 3 slots fixos e barra de widgets disponíveis rolável
# Versão 0.0.9-beta
# =============================================================================
...
class Dashboard(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # Barra de disponíveis com scroll horizontal
        self.available_scroll = ctk.CTkScrollableFrame(self, fg_color="transparent", orientation="horizontal", height=50)
        self.available_scroll.pack(fill="x", pady=5)
    ...
    def load_state(self):
        ...
        for wid in available_ids:
            ...
            if w not in slot_widgets:
                self.available_widgets.append(w)
        for w in WIDGET_TYPES:
            if w not in slot_widgets and w not in self.available_widgets:
                self.available_widgets.append(w)
    ...
    def _update_available_buttons(self):
        for child in self.available_scroll.winfo_children():
            child.destroy()

        for widget in self.available_widgets:
            btn = ctk.CTkButton(self.available_scroll, text=f"➕ {widget['name']}",
