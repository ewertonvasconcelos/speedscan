...
class SpeedScanPro(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self.sidebar, text="SPEEDSCAN", 
                     font=("Inter", 24, "bold"), text_color="#38bdf8").grid(row=0, column=0, padx=20, pady=40)
    ...
    def create_nav(self, text, row, icon, cmd):
        btn = ctk.CTkButton(self.sidebar, text=f"{icon}  {text}", 
                            anchor="w", height=45, fg_color="transparent",
                            hover_color="#334155", font=("Inter", 15), command=cmd)
    ...
    def show_dashboard(self):
        ...
        items = [
            ...
            ("Limpar Cache", "rm -rfv ~/.cache/*", "#b91c1c", "🧹"),
            ("Otimização", "sync; echo 3 > /proc/sys/vm/drop_caches", "#15803d", "🚀"),
            ("Limpar Apps", "dnf clean all || apt clean", "#334155", "📦"),
            ("Uso de Disco", "df -h", "#7e22ce", "📊")
        ]

        r, c = 0, 0
        for name, cmd, color, icon in items:
            card = ctk.CTkButton(self.grid_btns, text=f"{icon}\n\n{name}", width=230, height=130, corner_radius=22,
                                 fg_color=color, font=("Inter", 16, "bold"),
        ...
        self.toggle_btn = ctk.CTkButton(self.header_bar, text="Detalhes ▲", width=120, height=32, 
                                        fg_color="transparent", text_color="#94a3b8",
                                        hover_color="#1e293b", font=("Inter", 14, "bold"), 
                                        command=self.toggle_console)
        self.toggle_btn.pack()

        self.log_box = ctk.CTkTextbox(self.content, fg_color="#1e293b", text_color="#10b981", 
                                      font=("Monospace", 12), state="disabled", height=250)
    ...
    def run_cmd(self, cmd):
        try:
            # Iniciamos o processo com pkexec
            process = subprocess.Popen(f"pkexec bash -c '{cmd}'", shell=True, 
                                      stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1)
            
            self.log_box.configure(state="normal")
            self.log_box.delete("0.0", "end")
            
            # Lógica de espera: monitoramos a saída linha por linha
            first_output = True
            while True:
                ...
                if line:
                    # Se recebemos QUALQUER linha, significa que a senha foi aceita
                    if first_output and not self.details_btn_exists:
                        # Usamos after para atualizar a interface a partir da thread do comando
            ...
            if stderr:
                # Se houver erro (como senha errada ou cancelado), mostramos aqui
    ...
    def reveal_details_button(self):
        """Revela o botão de detalhes apenas quando chamado."""
        self.header_bar.grid(row=2, column=0, sticky="e", padx=40, pady=(20, 0))
        self.details_btn_exists = True

    def show_settings(self):
        self.clear_frame()
        ctk.CTkLabel(self.content, text="Ajustes", font=("Inter", 28, "bold")).pack(pady=40)

    def show_about(self):
        self.clear_frame()
        ctk.CTkLabel(self.content, text="Sobre", font=("Inter", 28, "bold")).pack(pady=40)
        ctk.CTkLabel(self.content, text="SpeedScan Pro\nEwerton Vasconcelos", font=("Inter", 18)).pack()
