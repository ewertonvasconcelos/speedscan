...
© 2026 Ewerton Vasconcelos. {self._("All rights reserved.")}"""
    ...
    def _run_dhclient(self, log):
        ...
        self._run_subprocess(["sudo", "dhclient", "-v"], log, tag=tag, use_sudo=True)
    ...
    def _run_update(self, log):
        ...
        self._run_subprocess(["sudo", "apt", "update"], log, tag=tag, use_sudo=True)
    ...
    def _run_firmware(self, log):
        self._run_subprocess(["dmesg", "|", "grep", "-i", "firmware"], log, tag=tag, shell=True)

    def _run_video_drv(self, log):
        self._run_subprocess(["lspci", "|", "grep", "-i", "vga"], log, tag=tag, shell=True)

    def _run_net_drv(self, log):
        self._run_subprocess(["lspci", "|", "grep", "-i", "network"], log, tag=tag, shell=True)
    ...
    def _change_dns(self, dns_ip, log):
        ...
        if hasattr(self, "action_mapper"):
            ...
            if cmd:
                self._run_subprocess(cmd, log, tag=tag, shell=True)
            else:
